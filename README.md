...
The `tweets: [Tweet.schema]` line sets up the embedded data association. The `[]` tells the schema to expect a collection, and `Tweet.schema` tells the schema that the collection will hold *embedded* documents of type `Tweet`.
